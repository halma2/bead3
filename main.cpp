...
struct Ablak : public wingui{
	...
	Ablak(const int& xx, const int& yy):wingui(xx,yy) {
		...
		s2 = new staticText(this,XX-120-40,30,120,30,"Jatekos2-z�ld");
    ...
    void kovi_kor(){
		...
		jatekos_kore_jelzo->write("Jatekos"+to_string(jatekos1_kore+1)+" k�re");
    }
	...
	void lerakas(int hova){
		...
		if (talalt_sorozat()){
			...
			jatekos_kore_jelzo->write("D�ntetlen!");
			for (int i = 0; i < 7; i++){
				oszlopgombok->at(i)->enabled = false;
				restartGomb->enabled = true;
				resetGomb->enabled = true;
